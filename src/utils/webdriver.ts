...
): ThenableWebDriver => new Builder().setChromeOptions(options).forBrowser('chrome').build();
...
export const quit = async (webdriver: WebDriver): Promise<void> => {
  ...
  const timeoutMilliseconds = (Number(process.env.BROWSER_QUIT_SECONDS_TIMEOUT) * 1000) / 2;
